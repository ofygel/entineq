    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full mx-auto p-4 space-y-4">
      <div className="glass rounded-2xl p-4">
        ...
        <div className="space-y-3">
          <input value={from} onChange={e=>setFrom(e.target.value)} placeholder="Забрать (адрес/2ГИС)" className="w-full rounded-xl px-4 py-3 bg-white/10 border border-white/20 outline-none" />
          <input value={to} onChange={e=>setTo(e.target.value)} placeholder="Доставить (адрес/2ГИС)" className="w-full rounded-xl px-4 py-3 bg-white/10 border border-white/20 outline-none" />
