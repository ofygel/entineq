...
const submit = () => { const order = createOrder({ type: 'TAXI', from, to, distanceKm, comment, priceEstimate: price }); openModal(`order-created-${order.id}`); };
return (
...
<input value={from} onChange={e=>setFrom(e.target.value)} placeholder="Откуда (2ГИС ссылка/адрес)" className="w-full rounded-xl px-4 py-3 bg-white/10 border border-white/20 outline-none" />
<input value={to} onChange={e=>setTo(e.target.value)} placeholder="Куда (2ГИС ссылка/адрес)" className="w-full rounded-xl px-4 py-3 bg-white/10 border border-white/20 outline-none" />
<div>
